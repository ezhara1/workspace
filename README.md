...
- For Qwen3.5 models, direct/no-thinking responses are enabled by default in manual mode (`ENABLE_THINKING=false`). `run_manual.py` launches native `llama-server` with `--jinja --reasoning-budget ${LLAMA_REASONING_BUDGET_NO_THINKING:-0} --chat-template-kwargs '{"enable_thinking": false}'`. Set `ENABLE_THINKING=true` to send `{"enable_thinking": true}` and use `LLAMA_REASONING_BUDGET_THINKING` (default `1024`).
...
3. The launcher will then pass:
   `--chat-template-kwargs '{"enable_thinking": true}'` and `--reasoning-budget ${LLAMA_REASONING_BUDGET_THINKING:-1024}`.
...
nohup /workspace/llama.cpp/build/bin/llama-server \
  ...
  --threads "${THREADS:-8}" \
  --jinja \
  --reasoning-budget "${LLAMA_REASONING_BUDGET_NO_THINKING:-0}" \
...
- `LLAMA_REASONING_BUDGET_NO_THINKING` / `LLAMA_REASONING_BUDGET_THINKING`: reasoning token budget used by `llama-server` for each mode
